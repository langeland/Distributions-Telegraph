...
class TelegraphController extends ActionController
{
     ...
     */
    public function deliverTelegramsAction(string $token, string $type)
    {
        $token = $this->tokenRepository->findOneByToken($token);
        if ($token === null) {
            return 'invalid token';
        ...
        $telegrams = $this->telegraphService->getTelegramsByToken($token, $type, null, true);
     ...
     * @throws \Neos\Flow\Persistence\Exception\IllegalObjectTypeException
     */
    public function listTelegramsAction(string $token, string $type)
    {
        $token = $this->tokenRepository->findOneByToken($token);
        if ($token === null) {
            return 'invalid token';
        ...
        $telegrams = $this->telegraphService->getTelegramsByToken($token, $type, null, false);
        $this->view->assign('value', $telegrams);
    }
