...
class TelegramService
{
    ...
    public function convert(string $message): string
    {
        ...
        $returnValue = 0;
        exec(FLOW_PATH_ROOT . 'bin/phantomjs --disk-cache=false ' . implode(' ', $arguments),$output, $returnValue);

        if($returnValue !== 0) {
            throw new \Exception('PhantomJS failed to convert the telegram. Error: ' . implode(PHP_EOL, $output), 1727165135);
        }
